...
class Validator {
	 ...
	 */
	function capture(array $inputs, array $constraints) {

		// validate the input data
		if (!is_array($inputs)) {
			throw new \Exception("`inputs` param expecting an array, string given. From method capture of Validator Class");
		}

		// convert the input data to dot notation
		$inputs = array_merge($this->arrayToDotNotation($inputs), $inputs);

		// loop through the constraints and validate the input data
		foreach($constraints as $name => $validator) {
			// If the input is required, it applies the validation methods specified in the constraints
			if(isset($constraints[$name]['required']) && $constraints[$name]['required'] === true) {
				foreach($validator as $method => $params) {
					if(method_exists($this, $method)) {
						...
						$this->input[$name] = $this->$method(name: $name, input: $inputs[$name], params: $params);
	...
	function getValidatedData() { return $this->dotNotationToArray($this->input); }
	...
	*/
	...
	function arrayToDotNotation($arr) {
		return array_to_dot_notation($arr);
	}

	function dotNotationToArray($arr) {
		return dot_notation_to_array($arr);
	}
