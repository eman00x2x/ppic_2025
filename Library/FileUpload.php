...
class FileUpload 
{
    ...
    private $settings = [
        "multiple" => false,
        "temp_folder" => "/Cdn/images/temporary",
        "temp_url" => "/images/temporary",
        "destination_folder" => "/Cdn/images",
        "final_url" => "",
        "file_type" => "image",
    ...
    protected function setMaxFileSize(string $file_type, string $file_size): void {
        ...
        $this->allowed_file_sizes[$file_type] = $this->convertfileSize($file_size);
        $this->settings['file_max_size'] = $this->allowed_file_sizes[$file_type];
    }
    ...
    protected function setAllowedMimeTypes($file_type): void {
        ...
        $this->mime_types = self::ALLOWED_MIME_TYPES[$file_type];
    ...
    function multipleFiles(array $data, array $options) {
		...
		foreach ($files as $file) {
			...
			if ($handle->uploaded) {
				...
				$handle->allowed = $this->mime_types;
				$handle->no_script = false;
				
				if(isset($options['image_resize'])) {
					$handle = $this->resizeImage($options['image_resize'], $handle);
				}
				...
				$this->results[] = [
                    "status" => 2,
                    "message" => "There was an error uploading your file ".$file['name']."."
                ];
			}
    ...
    function file(array $data, array $options) {
        ...
        if ($handle->uploaded) {
            ...
            $this->results = [
                "status" => 2,
                "message" => "Your file ".$file['name']." cannot be uploaded. Select another file for uploading."
            ];
        }
